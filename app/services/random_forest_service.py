...
class RandomForestForecastService:
    ...
    def load_data(self):
        try:
            ...
            self.avg_growth_rates = {
                ...
                'renewables_growth': self.df['renewables_share_energy'].pct_change().mean()
            }
            ...
            self.df['population_growth_feature'] = self.avg_growth_rates['population_growth']
            self.df['renewables_growth_feature'] = self.avg_growth_rates['renewables_growth']
            ...
            self.df['renewables_impact'] = self.df['renewables_share_energy'] * self.df['gdp_per_capita']
            
            # Trend features
            self.df['trend_5yr'] = self.df['y'].shift(1).rolling(window=5, min_periods=1).apply(
    ...
    def train_model(self, use_cv=True):
        ...
        feature_cols = [
            # Time features
            'year', 'year_index', 'year_squared', 'year_cubic',
            'year_sin', 'year_cos',
            ...
            'y_rolling_min_3', 'y_rolling_max_3',
            'trend_5yr',
            
            # Economic features
            'population', 'gdp', 'renewables_share_energy',
            'gdp_growth_feature', 'population_growth_feature', 'renewables_growth_feature',
            
            # Interaction features
            'gdp_per_capita', 'energy_intensity', 'renewables_impact'
        ]
    ...
    def forecast(self, years_ahead=10):
        ...
        for tree_idx, tree in enumerate(self.model.estimators_):
            ...
            for i in range(1, years_ahead + 1):
                ...
                pop = self.last_row['population'] * (1 + self.avg_growth_rates['population_growth'] * pop_factor) ** i
                gdp = self.last_row['gdp'] * (1 + self.avg_growth_rates['gdp_growth'] * gdp_factor) ** i
                renew = self.last_row['renewables_share_energy'] * (
                ...
                for window in [3, 5, 7]:
                    temp_df[f'y_rolling_mean_{window}'] = temp_df['y'].shift(1).rolling(window=window, min_periods=1).mean()
                    temp_df[f'y_rolling_std_{window}'] = temp_df['y'].shift(1).rolling(window=window, min_periods=1).std()
                    temp_df[f'y_rolling_min_{window}'] = temp_df['y'].shift(1).rolling(window=window, min_periods=1).min()
                    temp_df[f'y_rolling_max_{window}'] = temp_df['y'].shift(1).rolling(window=window, min_periods=1).max()
                
                temp_df['gdp_growth_feature'] = self.avg_growth_rates['gdp_growth']
                temp_df['population_growth_feature'] = self.avg_growth_rates['population_growth']
                temp_df['renewables_growth_feature'] = self.avg_growth_rates['renewables_growth']
                ...
                temp_df['renewables_impact'] = temp_df['renewables_share_energy'] * temp_df['gdp_per_capita']
                
                temp_df['trend_5yr'] = temp_df['y'].shift(1).rolling(window=5, min_periods=1).apply(
                    lambda x: np.polyfit(range(len(x)), x, 1)[0] if len(x) >= 2 else 0,
                    raw=False
                )
