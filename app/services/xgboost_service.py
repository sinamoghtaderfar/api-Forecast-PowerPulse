...
class XGBoostForecastService:
    ...
    def train_model(self):
        ...
        feature_cols = [
            ...
            'year_index', 'year_squared',
            'gdp_per_capita', 'energy_intensity'
        ]
            ...
            for i in range(1, years_ahead + 1):
                ...
                for window in [3, 5]:
                    temp_df[f'y_rolling_mean_{window}'] = (
                        temp_df['y'].shift(1).rolling(window=window, min_periods=1).mean()
                    )
                    temp_df[f'y_rolling_std_{window}'] = (
                        temp_df['y'].shift(1).rolling(window=window, min_periods=1).std()
                    )
